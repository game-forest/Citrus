...
namespace Tangerine.UI.Timeline
{
	...
	{
		public static void Decorate(SceneItem item)
		{
			var view = item.Components.GetOrAdd<RowView>();
			if (view.IsGridRowViewCreated) {
				return;
			}
			if (item.Components.TryGet<Core.Components.AnimatorSceneItem>(out var propRow)) {
			...
			} else if (item.Components.Contains<Core.Components.AnimationTrackSceneItem>()) {
