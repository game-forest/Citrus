...
namespace Tangerine.UI.SceneView.Utils.PolygonMesh
{
	...
	internal static class TopologyDataListExtenstions
	{
		...
		public static bool SequenceEquals(this List<int> list, List<int> other)
		{
			var isEqual = list.Count == other.Count;
			if (isEqual) {
				for (var i = 0; i < list.Count; ++i) {
					isEqual &= list[i] == other[i];
				}
			}
			return isEqual;
		}
	...
	internal class TopologyDataProvider
	{
		private class TopologyDataKey : IEquatable<TopologyDataKey>
		{
			...
			{
				IndexBuffer = new List<int>(topology.IndexBuffer);
			}
			...
			public override int GetHashCode()
			{
				unchecked {
					...
					hash = (hash * 728392183) ^ IndexBuffer.GetHashCode();
					foreach (var index in IndexBuffer) {
						hash = hash * 101 + index.GetHashCode();
					}
					return hash;
				}
			}
			...
			{
				return IndexBuffer.SequenceEquals(other.IndexBuffer);
			}
		}
		...
		public TopologyData ExtractData(ITopology topology)
		{
			...
			return new TopologyData(vertices.ToList(), edges.ToList(), faces.ToList());
		}
	}
}
