	...
	public class WidgetsPivotMarkPresenter
	{
		...
		private void RenderWidgetsPivotMark(Widget canvas)
		{
			...
			foreach (var widget in widgets) {
				...
				var p = sceneView.CalcTransitionFromSceneSpace(
					sceneView.Frame).TransformVector(widget.GlobalPivotPosition
				);
		...
		public static IEnumerable<Widget> WidgetsWithDisplayedPivot() =>
			Core.Document.Current.ContainerChildNodes().Editable().OfType<Widget>()
