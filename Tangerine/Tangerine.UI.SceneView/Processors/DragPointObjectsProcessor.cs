...
namespace Tangerine.UI.SceneView
{
	public class DragPointObjectsProcessor : ITaskProvider
	{
		...
		public IEnumerator<object> Task()
		{
			while (true) {
				...
				if (Utils.CalcHullAndPivot(pointObjects, out var hull, out _)) {
					if (SceneView.HitTestControlPoint((hull.V1 + hull.V3) / 2f)) {
		...
		private IEnumerator<object> Drag(List<PointObject> pobjects)
		{
			using (Document.Current.History.BeginTransaction()) {
				...
				while (SceneView.Input.IsMousePressed()) {
					...
					if (
						...
						&& (curMousePos - iniMousePos).Length > 5 / SceneView.Scene.Scale.X
					) {
					...
					if (dragDelta != Vector2.Zero) {
						for (int i = 0; i < pobjects.Count; i++) {
							...
							if (parent != null &&
								...
								var skinningRelativeInversedTransform = boneArray.CalcWeightedRelativeTransform(
									pobjects[i].SkinningWeights
								).CalcInversed();
								var newPosition = (
									(pobjects[i].TransformedPosition + dragDelta)
										* localToParent
										* skinningRelativeInversedTransform
										* localToParent.CalcInversed()
									- pobjects[i].Offset
								) / parent.Size;
								SetPosition(pobjects[i], newPosition);
								if (SceneView.Input.IsKeyPressed(Key.Control) && pobjects[i] is DistortionMeshPoint) {
		...
		private void SetUV(DistortionMeshPoint pointObject, Vector2 value)
		{
			Core.Operations.SetAnimableProperty.Perform(
				pointObject, nameof(DistortionMeshPoint.UV), value, CoreUserPreferences.Instance.AutoKeyframes
			);
		}
		...
		{
			Core.Operations.SetAnimableProperty.Perform(
				po, nameof(PointObject.Position), value, CoreUserPreferences.Instance.AutoKeyframes
			);
		}
	}
}
