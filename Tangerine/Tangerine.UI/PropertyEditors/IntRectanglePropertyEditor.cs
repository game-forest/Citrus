	...
	public class IntRectanglePropertyEditor : CommonPropertyEditor<IntRectangle>
	{
		...
		public IntRectanglePropertyEditor(IPropertyEditorParams editorParams) : base(editorParams)
		{
			...
			editorAX.AddLateChangeWatcher(
				currentAX, v => editorAX.Text = v.IsDefined ? v.Value.ToString() : ManyValuesText
			);
			editorAY.AddLateChangeWatcher(
				currentAY, v => editorAY.Text = v.IsDefined ? v.Value.ToString() : ManyValuesText
			);
			editorBX.AddLateChangeWatcher(
				currentBX, v => editorBX.Text = v.IsDefined ? v.Value.ToString() : ManyValuesText
			);
			editorBY.AddLateChangeWatcher(
				currentBY, v => editorBY.Text = v.IsDefined ? v.Value.ToString() : ManyValuesText
			);
