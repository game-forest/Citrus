...
namespace Tangerine.UI
{
	public class RectanglePropertyEditor : CommonPropertyEditor<Rectangle>
	{
		...
		public RectanglePropertyEditor(IPropertyEditorParams editorParams) : base(editorParams)
		{
			...
			editorAX.AddLateChangeWatcher(currentAX, v => editorAX.Text = v.IsDefined ? v.Value.ToString("0.###") : ManyValuesText);
			editorAY.AddLateChangeWatcher(currentAY, v => editorAY.Text = v.IsDefined ? v.Value.ToString("0.###") : ManyValuesText);
			editorBX.AddLateChangeWatcher(currentBX, v => editorBX.Text = v.IsDefined ? v.Value.ToString("0.###") : ManyValuesText);
			editorBY.AddLateChangeWatcher(currentBY, v => editorBY.Text = v.IsDefined ? v.Value.ToString("0.###") : ManyValuesText);
