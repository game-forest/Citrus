...
namespace Lime.Graphics.Platform
{
	...
	internal static unsafe class PlatformTexture2DExtensions
	{
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, T data)
			where T : unmanaged
		{
		...
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, ref T data)
			where T : unmanaged
		{
			GraphicsUtility.CalculateMipLevelSize(
				level, texture.Width, texture.Height, out var levelWidth, out var levelHeight
			);
		...
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, T[] data, int startIndex)
			where T : unmanaged
		{
			GraphicsUtility.CalculateMipLevelSize(
				level, texture.Width, texture.Height, out var levelWidth, out var levelHeight
		...
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, int x, int y, int width, int height, T data)
			where T : unmanaged
		{
		...
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, int x, int y, int width, int height, ref T data)
			where T : unmanaged
		{
			fixed (T* p = &data) {
		...
		public static void SetData<T>(
			this IPlatformTexture2D texture, int level, int x, int y, int width, int height, T[] data, int startIndex)
			where T : unmanaged
		{
